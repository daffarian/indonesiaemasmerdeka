...
import addPrivacyPolicy from "@/lib/action/addPrivacyPolicy";
import Link from "next/link";

export default function CreatePrivacyPolicyForm({ className }: { className?: string }) {
  // Quil
  const { quill, quillRef } = useQuill();

  const handelSubmit = (formData: FormData) => {
    addPrivacyPolicy(formData, quill?.getSemanticHTML());
  };
