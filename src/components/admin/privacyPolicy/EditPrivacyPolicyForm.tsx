...
// edit faq
import updatePrivacyPolicy from "@/lib/action/updatePirvacyPolicy";
...
export default function EditPrivacyPolicyForm({
  className,
  privacyPolicy,
}: {
  className?: string;
  privacyPolicy: PrivasiPolicy;
}) {
  // Quil
  ...
  useEffect(() => {
    if (quill) {
      quill.clipboard.dangerouslyPasteHTML(`${privacyPolicy.content}`);
    }
  }, [quill]);
  
  const handelSubmit = (formData: FormData) => {
    updatePrivacyPolicy(formData, privacyPolicy.id, quill?.getSemanticHTML());
  };
        ...
        <div className="flex flex-col gap-5 w-full">
          ...
          <div className="flex flex-col">
            ...
            <input
              ...
              defaultValue={privacyPolicy.title}
              required
            />
          </div>
          {/*Question Stop */}
