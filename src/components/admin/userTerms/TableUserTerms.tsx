...
import { UserTermsTableProps } from "@/types/TableProps";
import Link from "next/link";

// max words
import { setMaxWords } from "@/lib/utils";
import DeleteButtonUserTerms from "./DeleteButtonUserTerms";

const TableUserTerms = ({ data, className }: UserTermsTableProps) => {
        ...
        {data && data.length > 0 ? (
          data?.map((row, rowIndex) => (
            <tr
              ...
              key={rowIndex}
            >
              <td className="px-6 py-4">{setMaxWords(row.title, 10)}</td>
              <td
              ...
              <td className="px-6 !h-full text-nowrap">
                <div className="flex flex-row justify-start items-center gap-2">
                  {/* Button Delete Start */}
                  <DeleteButtonUserTerms id={row.id} />
                  {/* Button Delete Stop */}

                  <Link
                    href={`/dashboard/syarat-pengguna/${row.id}/edit`}
