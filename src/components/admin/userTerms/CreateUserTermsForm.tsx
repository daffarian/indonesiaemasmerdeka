...
// submit user terms
import addUserTerms from "@/lib/action/addUserTerms";

import Link from "next/link";

export default function CreateUserTermsForm({ className }: { className?: string }) {
  // Quil
  const { quill, quillRef } = useQuill();

  const handelSubmit = (formData: FormData) => {
    addUserTerms(formData, quill?.getSemanticHTML());
  };
