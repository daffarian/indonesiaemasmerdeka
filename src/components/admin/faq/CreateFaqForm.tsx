...
// submit article
import addFaq from "@/lib/action/addFaq";
import Link from "next/link";

export default function CreateFaqForm({ className }: { className?: string }) {
  // Quil
  const { quill, quillRef } = useQuill();

  const handelSubmit = (formData: FormData) => {
    addFaq(formData, quill?.getSemanticHTML());
  };
  ...
  return (
    <form action={handelSubmit} className="flex flex-col gap-5">
      ...
      <div>
        <label htmlFor="editor">Jawaban</label>
        <div
